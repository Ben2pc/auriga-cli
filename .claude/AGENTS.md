...
> The root `AGENTS.md` is the **product** (installed to user projects). This file guides development of auriga-cli itself.
...
| `AGENTS.md` / `AGENTS.en.md` | Manual | Workflow templates (the product). **Must be edited in tandem** — both languages must stay in sync |
...
- **Bump rule**: bump CLI version (`package.json`) before merging any PR that changes **user-visible state**.
  - **Bump triggers** (any of these touched):
    ...
    - `AGENTS.md` / `AGENTS.en.md` — workflow template, fetched at runtime
  ...
  - **Exempt** (no bump needed):
    - `.claude/AGENTS.md` (this dev guide — not shipped, not fetched)
