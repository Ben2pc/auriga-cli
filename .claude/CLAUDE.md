...
- **Bump rule**: bump CLI version (`package.json`) before merging any PR that changes **user-visible state** — code under `src/`, the workflow template (`CLAUDE.md` / `CLAUDE.zh-CN.md`), or any data registry shipped to users (`skills-lock.json`, `.claude/plugins.json`, `.claude/hooks/hooks.json`, hook script payloads, vendored skill content under `.agents/skills/`). The npm tarball's `dist/catalog.json` is built from these — if any of them changes, the tarball changes. Skip the bump only for genuinely internal edits: dev guide (`.claude/CLAUDE.md`), repo `README.md` / `README.zh-CN.md`, tests, CI configs, this section. Why: the runtime pins content fetch to `v<package.version>`, AND `dist/catalog.json` is frozen in the tarball — without a version bump + tag, merged content changes are invisible to `npx` users. Releases are cheap (CI auto-publishes on tag push); spend the version number rather than the user confusion.
