...
领域模型不要把领域语义、设计变更和代码载体压进一个含混的“概念”列。先用模型清单标明每个领域概念的变更状态与代码映射，再按概念分别写职责与行为、同一性判断、状态与存续范围，以及有效状态规则的成立边界；已有代码实体给出仓库相对路径，本次新增实体给出预计路径，没有独立代码载体时明确写为抽象概念。尚未决定是否落为代码实体的项目必须标为待确认，并进入人工评审重点。
