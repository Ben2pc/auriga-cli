...
- 领域模型中的概念、职责、变更状态和代码映射；每个概念都要明确是已有保留、已有调整、本次新增还是计划移除，并区分无独立代码实体的抽象概念与类、接口、文件等具体代码载体；需要时再逐项说明同一性判断、状态与存续范围，以及有效状态规则的成立边界。不涉及领域模型时明确跳过。
...
领域模型不要把领域语义、设计变更和代码载体压进一个含混的“概念”列。先用模型清单标明每个领域概念的变更状态与代码映射，再按概念说明职责、同一性、状态和规则；已有代码实体给出仓库相对路径，本次新增实体给出预计路径，没有独立代码载体时明确写为抽象概念。尚未决定是否落为代码实体的项目必须标为待确认，并进入人工评审重点。完整字段语义、占位形态和排除项以 `references/arch-design-template.md` 的 `Domain Model` 章节为唯一详细契约，本技能只保留执行摘要与入口。
