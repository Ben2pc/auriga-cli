...
description: 已确认改动需要协调多个完整结果、交付依赖或迁移中间状态时使用；单一明确结果不因跨文件而触发。
---
...
- 有现成计划时复用其内容，按完整单元定义补齐结果、依赖和完成证据；拆分不完整时调整。
...
将拆分结果记到对应规格目录的 `implementation-plan.md`；没有现成目录时使用 `docs/specs/<topic>/`，已有实施计划时就地更新，不另建清单。记录可使用下表，不要求改成固定格式：
