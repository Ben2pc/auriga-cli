---
name: spec-design
description: 新增或改变外部可见行为，或用户要求需求探索、规格设计时使用；基于实际代码与产品事实澄清目标、范围和验收契约。
---
...
1. 用 `git rev-parse --show-toplevel` 确认仓库根，读取适用于目标路径的 `AGENTS.md` 和相关项目资料。已读取且仍有效的材料直接复用，仅在相关事实变化或出现缺口时补查。
