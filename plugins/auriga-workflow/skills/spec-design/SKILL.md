...
3. 用户确认后按工作流交接：技术结构不显然、需要重划边界或需要领域建模时先进入 `arch-design` 并取得人工确认；任务只有单一明确结果、没有未决的产品或架构决定、无需跨会话跟踪且无需多个完整实施单元时可以直接进入编码前准备；其余任务按工作流模板复用或选择计划载体，再进入实现。用户明确选择的 `goalify` 可以与任一计划载体组合。
