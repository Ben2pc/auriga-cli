...
4. **一次解决一个相互依赖的决定。** 每个问题给出推荐答案、理由和主要后果，帮助用户思考，不替用户批准。
...
根据 Phase A 的事实建立会改变最终用户结果的决策树，沿依赖关系一次解决一个决定。每个问题说明为什么需要决定、推荐答案及主要后果；每个答案都要检查是否仍与原始目标一致。不存在会改变用户结果的真实决策分支时不提问，直接给出推荐，不制造候选或选择仪式。
