...
安全、合规、严重故障和数据正确性等必做事项可以简化价值讨论，但仍要确认影响和优先级。两轮后证据仍不足时，推荐“先验证”或“暂缓”，不继续无限讨论，也不默认进入完整实现。

#### B2. 苏格拉底式需求对齐

根据 Phase A 的事实建立会改变最终用户结果的决策树，沿依赖关系一次解决一个决定。每个问题说明为什么需要决定、推荐答案及主要后果；不存在真实取舍时直接推荐，不制造候选或选择仪式。
...
3. 用户确认后按工作流交接：技术结构不显然、跨模块或需要领域建模时先进入 `arch-design` 并取得人工确认；快速开发流程满足三条谓词且不需要架构澄清时直接进入编码前准备；其余任务进入用户选择的计划方式。
