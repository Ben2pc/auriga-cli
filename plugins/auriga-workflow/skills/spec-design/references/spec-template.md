...
<本 spec **有意**留给 plan / 实现阶段决定的问题。不是未澄清需求歧义的停车场——若问题是关于"用户想要什么"且尚未回答，回 A2 澄清循环，不写这里。每条**必须**写明归属（plan / impl）与 deferred 理由；两者都写不出，就不是 open question。编号列表；没有则写"无"——这是正常结果。>
