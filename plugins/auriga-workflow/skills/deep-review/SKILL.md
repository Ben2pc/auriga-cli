...
- **重叠判定**：对照上述 11 位内置审查者，判断该自定义审查者的关切是否落在某位内置审查者的维度范围之内。以**语义判断**为主——比较自定义文件的 `Best for`、`Scope` 段与 `Checklist` 跟各内置维度的覆盖面；若自定义文件的元数据中手写了 `Extends: <内置审查者名>` 字段，则优先采信该字段、不再做语义猜测（`Extends` 值不是 11 位内置审查者之一时忽略它，回退语义判断）。`Extends` 是可选的、手填的——`reviewer-creator` 不会自动产出它。与内置审查者**重名**视为必然重叠，host 即同名内置审查者。
- **重叠 → 吸收**：判定为与某位内置审查者（host）重叠时，不再为该自定义审查者分派独立子代理；改为把它的 `Checklist` 与 worked scenarios（示例场景）原文并入 host 内置审查者的子代理提示词，作为「项目专属补充」由 host 一并审查。若 host 内置审查者本次未被触发（例如自定义审查者重叠 `performance`，但本次 PR 无 `perf` 标签），被吸收的内容随 host 一并不运行——重叠自定义审查者自身的 `Trigger` 不再独立生效。
- **不重叠 → 独立分派**：判定为引入一个**全新维度**（不落在任何内置维度范围内）时，解析其元数据中的 `Trigger` 字段，路由到对应的分派类别（A/B/C/D），作为独立审查者分派——此场景行为与之前一致。
