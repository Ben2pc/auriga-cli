...
**输出契约：** 将每个参考文件的 `Output contract` 章节原文传入子代理提示词——不要依赖默认值。每个审查者提示词（内置或 `docs/rules/review/` 自定义的）还必须在开头原文附上**审查者禁止事项前置说明**（见下方章节）——这些角色级约束统一适用于所有维度，集中在此处以便单次修改即可传达至每位审查者。
