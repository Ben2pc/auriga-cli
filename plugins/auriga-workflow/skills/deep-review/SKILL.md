...
多维度拉取请求审查器。主代理负责收集原始材料、完整审查差异、选择必要的专门审查者并综合结果；被选中的审查维度在彼此隔离的全新上下文中独立判断。
...
内置审查者位于 `references/reviewers/`。主代理只读取每个文件的 YAML frontmatter 判断其 `best_for`、`value`、候选信号、推理投入和工具需求；选择后把文件**绝对路径**交给审查代理，由审查代理自读正文。若执行环境无法解析仓库绝对路径，才内联该文件正文。内置审查者的 `trigger` 只表示候选信号：`always` 表示通用候选，不代表必选。
...
主代理先完整检查原始差异和权威要求，再按具体风险、审查者的 `best_for` 与 `value`、已有可信证据和额外审查成本选择专门审查者。候选信号不是固定路由表；主代理可以选择未命中标签但有明确价值的审查者，也可以跳过已命中但不会增加判断质量的审查者。`correctness` 同时覆盖正常、边界和失败路径，不再单独分派鲁棒性审查者。

主代理可以不选择任何内置或标签候选的项目审查者，但仍须自行完成全面审查并输出完整报告；零个专门审查者本身不视为审查缺口。只有 `trigger: always` 的项目审查者不得跳过。选择后记录简洁的选择依据，说明为什么这些专门维度能增加判断质量；没有选择时说明为什么主代理审查已经足够，不要求逐个解释所有未选审查者。

## 5. 独立执行

每个被选中的专门维度必须在**全新、干净的上下文**中运行：
...
主代理先完成自己的全面审查，再逐条复核专门审查者证据，按同一根因合并重复发现，同时保留所有来源。主代理自行发现的条目以 `(主代理)` 为来源；没有专门审查者时仍按相同契约形成结论。不要仅因置信度低就把条目移动到架构维度；证据不足但风险具体时进入“需要验证”，证据足够时按实际影响分类。
...
**信号**：<标签>  |  **已完成审查者**：<列表；没有时写“无（主代理完成全面审查）”>
**选择依据**：<为什么选择这些专门审查者；没有选择时说明主代理审查为何足够>
