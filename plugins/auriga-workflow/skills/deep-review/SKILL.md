...
frontmatter 的 `tools` 是审查者申请的最大权限，不是提示词建议。实际权限取“审查者 `tools`、主编排只读策略、运行时可执行限制”的交集，并通过宿主的允许列表、禁止列表或只读沙箱真正限制。若运行时无法限制工具且无法阻止外部写入，不分派该维度，记录审查缺口，不能用自然语言承诺冒充权限边界。
