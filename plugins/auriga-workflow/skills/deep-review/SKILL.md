...
内置 Agent 通过平台原生隔离与权限边界运行。宿主支持逐 Agent 的允许列表、禁止列表或只读沙箱时，应当用这些机制收紧权限；缺少逐 Agent 的细粒度工具允许列表，不能切换到外部 Agent，也不能停止内置评审。下面的 Reviewer Must-Not Preamble 仍作为审查任务契约。
