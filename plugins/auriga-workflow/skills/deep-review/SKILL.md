...
主代理丢弃子 reviewer 的局部编号，再按来源分组，相同来源的条目必须相邻；同一来源组内按对当前拉取请求的影响排序。多来源合并条目以第一来源作为分组键，同时在来源单元格保留全部来源。完成排序后再从 1 开始连续编号；阻断与非阻断分别使用 `阻断n`、`非阻断n`。所有分类章节始终保留；没有条目时直接写 `无。`，不输出空表或占位行。
