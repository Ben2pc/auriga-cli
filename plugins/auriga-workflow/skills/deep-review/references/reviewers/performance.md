...
- **最适合**：在延迟 / 内存 / 耗电量回归达到用户可感知阈值之前发现它们
- **触发**：tag:perf
- **推理档位**：workhorse
- **工具**：Read, Grep, Glob（只读）
- **价值**：性能回归是无声的——它们不会让测试失败，只有在用户投诉时才会被注意到
...
6. **iOS 特有**：动画过程中的视图层级变更；长列表用 `LazyVStack` vs `VStack`；主 Actor 跳转
7. **Android 特有**：Activity 重建成本；`Compose` 重组范围；`RecyclerView` 视图持有者复用
...
| 推荐关注 | 检测 |
|---|---|
| Web 渲染 | `.tsx` / `.jsx` / `useState` / `useEffect` / `setState` / 动画库 |
| 打包 | `package.json` 依赖变更；新引入大型库（`moment`、`lodash` 整包）、`webpack.config.js`、`vite.config.ts` |
| 移动端（iOS） | `.swift` / `UIKit` / `SwiftUI` / `DispatchQueue.main` / `URLSession` |
| 移动端（Android） | `.kt` / `Compose` / `ViewModel` / `Coroutine` / `OkHttp` |
| 后端热路径 | 路由处理器、中间件、请求循环、事件处理器（`onMessage`、`onRequest`） |
| 数据库查询 | 循环中的 ORM 调用、原始 SQL 变更、新增索引 / 迁移 |
| 长进程服务 | `setInterval` / `cron` / `EventEmitter.on` / 缓存初始化 |
