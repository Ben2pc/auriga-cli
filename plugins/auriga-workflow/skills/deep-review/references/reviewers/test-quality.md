...
场景 A 中的 §1–§8 评审标准与 `test-driven-development` 的行为导向、边界模拟和风险取舍保持一致：实现阶段由当前代理建立证据，审查阶段再由独立审查者检查遗漏。具体评审标准保留在本文件，避免把正式评审流程复制进精简技能。

在应用通用标准前，先检查仓库中的 `docs/rules/test/`。目录以仓库根为锚：用 `git rev-parse --show-toplevel` 解析仓库根，检索 `<仓库根>/docs/rules/test/`；当 cwd 不在仓库根、且 cwd 到仓库根之间存在更近的 `docs/rules/test/`（monorepo 子包）时，两层都收集——子包级规则视为对仓库级的补充/收窄，冲突时以子包级为准；非 git 仓库时回退为相对 cwd 检索。读取与当前模块、测试类型、fixture 或 runner 相关的规则；若目录不存在或没有相关文件，在摘要中说明「无项目专属测试规则」。任何违反项目测试规则的测试，或新增生产行为未按项目规则补测试的情况，都应作为本审查者发现上报。
...
静态阅读差异常常不足以对 §6（不稳定风险）和 §7（测试必须真的会失败）这类发现下定论。此时可以**运行仓库的单元测试**做经验验证，把判断从推测提升为证据——这是对静态检查清单的增强，不是替代。

- **允许**：运行仓库的单元测试套件或其中单个单元测试用例。典型用途——跑差异新增的测试确认它确实通过；对可疑的永远绿灯测试做反向验证；重复运行以探测不稳定。
- **禁止**：运行集成 / 端到端 / 浏览器 / 移动端测试，或任何会写文件、连网络、改数据库或耗时显著的测试。它们有副作用且慢，超出对抗式只读审查者的边界。只能由集成 / e2e 覆盖的场景，按静态阅读判断并照常标记。
- **绝不写入**：不修改被测代码或测试文件、不提交、不留下产物——运行测试纯为取证。

无法判断某测试是单元还是集成级时，按集成级对待、不运行。在摘要中注明哪些发现经过了实跑验证。
...
对于场景 A 发现，在描述前加上触发的评审标准章节（`§1` 到 `§8`），以便综合步骤按标准分组，并能反馈给后续测试驱动开发。对于场景 B 发现，指向未测试分支所在的**生产**文件 file:line，而非测试文件（测试尚不存在）。只有在真的没有发现任何问题时才返回 `"No findings."`。
