...
# 用法: assemble.sh <body.html> <output.html> [custom.css] [title] [lang] [figures.json]
...
for SRC in "$BODY" "$CUSTOM" "$FIGURES"; do
...
trap 'rm -f "$TMP" "$SAFE_FIGURES"' EXIT

# 正文只承载结构和已转义文本；图形数据走独立 JSON。校验失败时拒绝打开报告，
# 避免仓库里的 HTML/脚本片段被误当成报告活动内容执行。
node "$SCRIPT_DIR/validate-inputs.mjs" "$BODY" "$CUSTOM" "$FIGURES" "$SAFE_FIGURES"
