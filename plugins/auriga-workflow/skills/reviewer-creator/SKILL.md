...
根据规则的最终判断责任选择最接近的宿主；不是按文件类型机械选择。仓库证据足够明确时直接确定定位和触发条件；只有会改变 `extends` 或强制与候选语义的真实歧义才询问用户确认。

`always` 只用于几乎所有正式拉取请求都必须检查的项目契约，命中后必须执行；`tag:<标签>` 要描述差异中的事实或风险表面，命中后只把审查者加入候选，由主代理判断是否选择。
...
- 未验证 frontmatter 和触发条件的强制或候选语义就宣称创建完成。
