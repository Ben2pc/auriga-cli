...
- 主 Agent 按 `SKILL.md` 的共同流程完成当前资产核对，并把“已吸收 / 部分吸收 / 未吸收 / 未知”、当前资产证据和会话证据放进汇总输入。洞察 Agent 不自行读取工程文件。
...
报告面向人类阅读，正文使用派遣包指定的任务语言，表达自然、具体。避免直接翻译英文分析术语、连续堆叠抽象名词，或用内部流程术语代替实际发生的行为；例如直接写“先确认工具的正确用法”，不要写“工具契约探测”。标题优先说明具体做法、问题或改进方向，必要术语首次出现时用实际行为解释。
