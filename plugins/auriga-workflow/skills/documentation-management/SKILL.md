...
5. **Agent 文档是提示词和上下文工程资产。** 只保留会改变行为的目标、成功标准、约束、权限边界、工具路由、输出契约和停止条件；同一规则只写一次，不堆教学、口号、同义重复和过期示例。
...
- 使用结果优先的短指令：写清目标、成功标准、真实不变量、证据要求、授权范围和停止条件，让模型自行选择高效路径。
...
- Agent 文档额外检查：指令是否由正确作用域的 `AGENTS.md` 发现、无冲突、无重复，是否明确成功与停止条件，是否把纯人类叙事误挂到 Agent 上下文。
