...
- 分层维护 `AGENTS.md`：仓库根只放全局规则与导航；具有独立职责、命令或约束的子包在自己的根目录维护 `AGENTS.md`。子级只补充或收窄祖先规则，不复制继承内容。`CLAUDE.md` 只在目标 Claude Code 会话无法原生读取 `AGENTS.md` 或确有宿主专属指令时作为显式兼容入口，不默认逐层创建。
