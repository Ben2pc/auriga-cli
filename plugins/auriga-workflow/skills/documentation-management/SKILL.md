...
4. **先确定消费者。** 新文档必须明确服务人类、Agent，还是双方共同消费。命名即信号：`README.md`、`ONBOARDING.md`、`CONTRIBUTING.md` 等传统人类入口默认面向人类，其余 markdown 默认按 Agent 文档标准维护。共享不是默认答案；只有 schema、ADR 等共同事实或决策记录确实被双方使用时才采用共享资产，并分别设计发现入口。
