...
2. **一个事实只有一个当前信息源。** 其他入口只导航，不复制正文。代码、schema、配置或生成物已经能可靠表达的事实，不再手工维护第二份。
3. **先确定消费者。** 新文档必须明确服务人类、Agent，还是双方共同消费。共享不是默认答案；只有 schema、ADR 等共同事实或决策记录确实被双方使用时才采用共享资产，并分别设计发现入口。
4. **纯人类文档不挂到 `AGENTS.md`。** README、教程、背景叙事和只面向人的运行说明不因“Agent 也许有用”而进入 Agent 指令入口。Agent 真正需要的当前约束，应提炼成简短的 Agent 文档；Agent 确实需要阅读的共享事实或决策记录可以由对应作用域索引。
5. **区分 Agent 资料与 Agent 指令。** Agent 阅读的架构文档、接口契约、ADR 等工程资料沿用各自的文档结构；只有提示词、项目规则或标准操作流程（SOP）等直接控制 Agent 行为的指令，才按目标、成功标准、约束、权限边界、工具路由、输出契约和停止条件组织。
6. **归档记录保存当时事实。** worklog 和已被取代的决策记录通常不追赶当前实现；当前入口应指向新的有效事实。
7. **从证据写文档。** 读取当前代码、配置、接口和测试后再修改文档，不用旧文档互相证明。
...
- Agent 子文档由**最近的适用作用域**的 `AGENTS.md` 建立索引，并说明何时读取。多个子包共同使用时，提升到最近公共祖先；不要把局部上下文全部挂到仓库根。
