...
其他任务中，当前修改直接必要、行为不变且影响确定的低风险局部整理属于已授权范围。偶然发现可独立交付的重构时，先报告具体位置、维护成本、建议方向和预计影响范围，取得用户确认后再修改；不要把独立清理混入用户没有授权的改动。
...
- 保护网不足时，按 `test-driven-development` 选择验证证据；只有满足其永久测试门槛时才补充特征测试，否则使用与风险匹配的其他有效证据。
