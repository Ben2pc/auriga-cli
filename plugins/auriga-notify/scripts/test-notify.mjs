...
//
//     node /path/to/plugins/auriga-notify/scripts/test-notify.mjs
//
...
const ENTRY = path.join(HERE, "notify.mjs");
...
// AURIGA_NOTIFY_FORCE_BANNER=1 bypasses the focus check so the banner
// shows even if you're staring at the terminal you ran the test from.
// Without it, a focused terminal would only get the sound — which can
// read as "the test failed" when it's actually working as designed.
const child = spawn("node", [ENTRY], {
  stdio: ["pipe", "inherit", "inherit"],
  env: { ...process.env, AURIGA_NOTIFY_FORCE_BANNER: "1" },
});
...
child.on("exit", (code) => {
  if (code !== 0) {
    console.error(`✗ auriga-notify exited with code ${code}`);
    process.exit(code ?? 1);
  }
