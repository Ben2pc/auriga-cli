...
When `auriga-workflow` replaces a formerly standalone workflow skill, safe legacy cleanup runs only through `auriga-cli`. Installing the plugin directly with `claude plugins install` or `codex plugin add` bypasses provenance checks, replacement-payload verification, and cross-runtime preservation; run `npx -y auriga-cli install plugins --agent both --plugin auriga-workflow` at least once when migrating an existing setup. Codex plugins are user-level, so selecting Codex also checks managed user-level legacy copies even when `--scope project` is used; custom, locally modified, or otherwise unproven copies are preserved.
