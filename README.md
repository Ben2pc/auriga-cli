...
Of the four plugins maintained in this repository, `quality-gate-scaffolder` exposes a root `plugin.json` compatible with [Agent Plugins 1.0.0](https://agent-plugins.org/specification). Standard clients discover its portable components only from the fixed `skills/` and `mcp.json` locations. Hook plugins temporarily omit the root `plugin.json` because current Codex discovery can otherwise select the standard package and skip `hooks/hooks.json`; `auriga-workflow`, `session-instructions-loader`, and `auriga-notify` retain their applicable host-specific Claude Code, Codex, or Cursor native manifests and Hook registry. The Runtime column below lists hosts that can install each plugin. The CLI installer still targets Claude Code and Codex only.

Cursor indexes this repository through `.cursor-plugin/marketplace.json`, which currently lists `auriga-workflow` and `quality-gate-scaffolder`. Personal users add the GitHub repo with `cursor-agent plugin marketplace add https://github.com/Ben2pc/auriga-cli`, then install the listed plugins. Team or Enterprise plans can also add the same repo as a team marketplace from the Cursor dashboard.
...
| auriga-workflow | Claude Code / Codex / Cursor | Auriga's end-to-end engineering workflow. It covers evidence-first diagnosis, risk-matched verification and selective permanent tests, requirement and architecture clarification, incremental implementation, bounded autonomous execution with human gates, conditional local deep review, documentation management, and Git lifecycle enforcement. |
| quality-gate-scaffolder | Claude Code / Codex / Cursor | Scaffold stack-specific quality gates for Swift iOS, Kotlin Android, Python backend, TypeScript frontend, and Node tool repositories. |
