...
plugins/
  auriga-workflow/
                  仓库自有的 workflow plugin（Claude Code、Codex、Cursor），
                  ...
                  `.claude/skills/<name>` 软链——plugin 打包的 skills 由
                  marketplace 安装，再从标准固定的 `skills/` 位置发现。
  quality-gate-scaffolder/
                  仓库自有的 quality-gate skills plugin（Claude Code、Codex、Cursor）。
...
.claude-plugin/
  marketplace.json — 本仓库的 Marketplace manifest；列出
                     ...
                     auriga-notify，供 Claude Code 使用。
...
.cursor-plugin/
  marketplace.json — 本仓库发布给 Cursor 的 Marketplace manifest；列出
                     auriga-workflow + quality-gate-scaffolder。
                     Cursor 只索引这份清单，不会读 Claude / Codex 的
                     marketplace.json。
...
- **Skill 分类**：`skills.ts` 里的 `WORKFLOW_SKILLS` 是通过 `npx skills add` 安装的外部 workflow skills 的 curated 默认启用集合。`skills-lock.json` 里的其他内容都属于“recommended”（按需启用工具）。auriga-cli 自有的 workflow skills 现在放在 `plugins/auriga-workflow/`，通过 plugin 路径安装。
  ...
  - **编辑 auriga-cli 自有的 workflow skill**（`systematic-debugging`、`test-driven-development`、`incremental-impl`、`session-compound`、`spec-design`、`arch-design`、`code-simplify`、`goalify`、`deep-review`、`reviewer-creator`、`git-workflow`、`documentation-management`、`docent`）：编辑 `plugins/auriga-workflow/skills/<name>/` 下的源文件。自有 skills **不带** `.claude/skills/<name>` 或 `.agents/skills/<name>` 软链——plugin-bundled skills 通过 marketplace 安装，并从标准固定的 `skills/` 位置发现。不要把自有 skill 名字加回 `skills-lock.json` 或 `WORKFLOW_SKILLS`；面向用户的安装入口是 `auriga-workflow` plugin。若某次编辑改变了 skill 的**输出契约**（它会产出给下游 skill 消费的文件 / 字段 / 内容），要在同一次变更里更新它的 consumer skills——自有 skills 是一条流水线（`spec-design` → `test-driven-development` / `deep-review` → `incremental-impl`），契约变化如果不向下游传播，就会变成跨 skill drift。
  - **添加新的自有 workflow skill**：在 `plugins/auriga-workflow/skills/<new-name>/` 下新增，带上自己的 `SKILL.md`（如果 body 预计会超过 ~500 行，可再加 `references/` 做 progressive disclosure）。**不要** 在 `.claude/skills/` 或 `.agents/skills/` 下创建软链——各宿主从插件的 `skills/` 位置发现技能，原生清单继续补充运行时能力。更新 `auriga-workflow` 的原生 plugin manifests，以及 `.claude-plugin/marketplace.json` 里的 `auriga-workflow` description，这样新 skill 才会进入 `dist/catalog.json`。同步 `plugins/auriga-workflow/.cursor-plugin/plugin.json` 的身份字段，并保持 `"skills": "./skills/"`，否则远端索引可能扫不到全部技能。Cursor 市场条目只保留 `name`、`source` 和 `category`。只有当这个 skill 成为 workflow contract 的一部分时，才更新产品 workflow templates。
...
- **添加 auriga-cli 自有 plugin**（例如 `auriga-workflow`——skills + hooks——或 `quality-gate-scaffolder`——只有 skills）：在仓库根目录 `plugins/<name>/` 下编写。这个仓库本身就是真源。不含 Hook 的插件可提供符合 Agent Plugins 1.0.0 的根 `plugin.json`；标准清单顶层字段闭合，可移植组件只从固定的 `skills/` 与 `mcp.json` 发现，缺少组件位置合法。包含 Hook 的插件暂时不提供根 `plugin.json`，避免 Codex 选择标准入口后跳过 `hooks/hooks.json`；Claude Code 目标插件保留 `.claude-plugin/plugin.json`，Codex 目标插件保留 `.codex-plugin/plugin.json`，Cursor 目标插件保留 `.cursor-plugin/plugin.json`。可选 hook scripts（宿主 Hook 示例见 `plugins/auriga-workflow/`），可选 `skills/<skill-name>/SKILL.md`（可加 `references/`）。**`plugins/<name>/` 里的所有内容都会发给用户**——开发专用资源（tests、generators）要放在仓库根目录的 `tests/`。然后按目标运行时注册 marketplace：Claude Code 使用 `.claude-plugin/marketplace.json`（`"source": "./plugins/<name>"`），Codex 使用 `.agents/plugins/marketplace.json`，Cursor 使用 `.cursor-plugin/marketplace.json`（`metadata.pluginRoot` 为 `plugins`，`source` 为裸插件名），并在 `plugins/<name>/.cursor-plugin/plugin.json` 放一份 Cursor 原生清单。`extra_plugin_configs.json` 只用于像 `defaultOn` 这样的策略覆盖，不作为本地 plugin 的真源。用户通过 `npx auriga-cli` → Plugins 安装。
  - **旧独立技能不自动迁移**：plugin 安装路径不得扫描、改写或删除 `.claude/skills`、`.agents/skills` 和 skills 锁文件。团队成员确认插件技能生效后，通过 `npx skills remove <skill-name>` 或人工清理旧副本；内部小团队用直接通知协调，不在安装器中维护跨运行时、跨作用域的自动迁移状态机。
  - **双 Agent 版本（Claude Code + Codex）**：Codex 的 hook system 与 Claude Code 兼容 schema（`hooks.<Event>[].matcher + hooks[]` 结构，`${CLAUDE_PLUGIN_ROOT}` 也被 Codex 刻意镜像，以便开箱兼容，stdin/stdout 合同一致）。要把 plugin 同时注册到两个 marketplace：Claude Code 用 `.claude-plugin/marketplace.json`，Codex 用 `.agents/plugins/marketplace.json`。另外再加一份 `.codex-plugin/plugin.json`，写入 Codex 特有的更丰富 schema（`version`、`homepage`、`repository`、`license`、`keywords`、带 `displayName` / `category` / etc. 的 `interface` block）；`.claude-plugin/plugin.json` 保持最小，但也要镜像 `version`，方便升级比较器使用。`hooks/hooks.json` 内容相同，`scripts` 相同，README 也相同——canonical 示例见 `plugins/auriga-workflow/`。Codex 支持 hook `additionalContext`；block path（`exit 2 + stderr`，或 `permissionDecision: "deny"`）的行为完全一样。壳命令匹配器写成 `Bash|Shell|PowerShell|run_terminal_command`，写文件匹配器列出各宿主的文件修改工具名（含 Grok Build 的 `search_replace`）。命令是否匹配由 script 自己判断，并同时读取 `tool_input` / `toolInput`；不要再靠宿主专属的 `if` 或脚本内 `tool_name` 白名单做第二道门禁。Grok Build 的 `PostToolUse` 会丢弃标准输出，因此提交提醒和创建快照在该宿主上无法注入模型上下文，拦截路径仍走退出码 2。
  - **清单身份同步**：同一插件的所有适用原生清单必须保持 `name`、`version` 一致；存在根 `plugin.json` 时，它也必须与原生清单一致。根清单只表达 Agent Plugins 1.0.0 的可移植身份；`.claude-plugin/plugin.json`、`.codex-plugin/plugin.json` 与 `.cursor-plugin/plugin.json` 继续承载宿主专属字段。
...
| `plugins/<name>/` | 手动 | auriga-cli 自有 plugin 源码（例如 `plugins/auriga-workflow/`）。无 Hook 插件可用根 `plugin.json` 定义 Agent Plugins 1.0.0 可移植身份；Hook 插件暂时不提供根 `plugin.json`，由原生清单与 marketplace 定义宿主安装能力。标准组件仍只从固定的 `skills/` 与 `mcp.json` 发现。目录内的一切都会发给用户——开发专用资源（tests）放到仓库根目录 `tests/` |
...
| `.cursor-plugin/marketplace.json` | 手动 | 本仓库发布给 Cursor 的 plugin Marketplace manifest。条目只保留 `name`、`source`、`category`；身份字段以 `plugins/<name>/.cursor-plugin/plugin.json` 为准。不进 CLI 内容清单，只给 Cursor 索引 |
