...
- **Skill 分类**：`skills.ts` 里的 `WORKFLOW_SKILLS` 是通过 `npx skills add` 安装的外部 workflow skills 的 curated 默认启用集合。`skills-lock.json` 里的其他内容都属于“recommended”（按需启用工具）。auriga-cli 自有的 workflow skills 现在放在 `plugins/auriga-workflow/`，通过 plugin 路径安装。
  ...
  - **添加新的自有 workflow skill**：在 `plugins/auriga-workflow/skills/<new-name>/` 下新增，带上自己的 `SKILL.md`（如果 body 预计会超过 ~500 行，可再加 `references/` 做 progressive disclosure）。**不要** 在 `.claude/skills/` 或 `.agents/skills/` 下创建软链——Agent Plugins 1.0.0 从固定的 `skills/` 位置发现，宿主清单继续补充各运行时能力。更新 `auriga-workflow` 的根 `plugin.json` 与原生 plugin manifests，以及 `.claude-plugin/marketplace.json` 里的 `auriga-workflow` description，这样新 skill 才会进入 `dist/catalog.json`。若该插件也在 Cursor 市场列出，同步 `plugins/<name>/.cursor-plugin/plugin.json` 的身份字段，并保持 `"skills": "./skills/"`，否则远端索引可能扫不到全部技能。Cursor 市场条目只保留 `name`、`source` 和 `category`。只有当这个 skill 成为 workflow contract 的一部分时，才更新产品 workflow templates。
...
- **添加 auriga-cli 自有 plugin**（例如 `auriga-workflow`——skills + hooks——或 `auriga-notify`——只有 hooks）：在仓库根目录 `plugins/<name>/` 下编写。这个仓库本身就是真源。必需结构是符合 Agent Plugins 1.0.0 的根 `plugin.json`；标准清单顶层字段闭合，可移植组件只从固定的 `skills/` 与 `mcp.json` 发现，缺少组件位置合法。Claude Code 目标插件补 `.claude-plugin/plugin.json`，Codex 目标插件补 `.codex-plugin/plugin.json`；Hook 注册表放在 `hooks/hooks.json`，`interface`、Hook 路径等宿主专属覆盖字段放在适用的原生清单或标准 `extensions`，根清单不能伪造跨宿主支持。可选 hook scripts（宿主 Hook 示例见 `plugins/auriga-workflow/`），可选 `skills/<skill-name>/SKILL.md`（可加 `references/`）。**`plugins/<name>/` 里的所有内容都会发给用户**——开发专用资源（tests、generators）要放在仓库根目录的 `tests/`。然后按目标运行时注册 marketplace：Claude Code 使用 `.claude-plugin/marketplace.json`（`"source": "./plugins/<name>"`），Codex 使用 `.agents/plugins/marketplace.json`，Cursor 使用 `.cursor-plugin/marketplace.json`（`metadata.pluginRoot` 为 `plugins`，`source` 为裸插件名），并在 `plugins/<name>/.cursor-plugin/plugin.json` 放一份 Cursor 原生清单。`extra_plugin_configs.json` 只用于像 `defaultOn` 这样的策略覆盖，不作为本地 plugin 的真源。用户通过 `npx auriga-cli` → Plugins 安装。
  ...
  - **清单身份同步**：同一插件的根 `plugin.json` 与所有适用原生清单必须保持 `name`、`version` 一致。根清单只表达 Agent Plugins 1.0.0 的可移植身份；`.claude-plugin/plugin.json`、`.codex-plugin/plugin.json` 与 `.cursor-plugin/plugin.json` 继续承载宿主专属字段，不因新增标准入口而删除。
...
| `plugins/<name>/` | 手动 | auriga-cli 自有 plugin 源码（例如 `plugins/auriga-workflow/`）。根 `plugin.json` 定义 Agent Plugins 1.0.0 可移植身份，原生清单与 marketplace 定义宿主安装能力。目录内的一切都会发给用户——开发专用资源（tests）放到仓库根目录 `tests/` |
