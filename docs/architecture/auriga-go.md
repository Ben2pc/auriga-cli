...
| Progress visibility | **No prescribed echo format.** Each Agent has its own task tracker (Claude Code's `TaskCreate`/`TaskUpdate`, Codex's equivalent, etc.); auriga-go's job is to tell the Agent to record the current workflow step through that native tracker, which is finer-grained than any single-line echo and is already the user's interrupt surface. ship's only required transcript format is `<ship-done>Ready\|Blocked</ship-done>` as the final assistant text — that's the Stop-hook signal. Iteration count for ship is held in `.claude/auriga-go-ship.local.md` and survives `/clear`. |
...
- **Autonomy tension** with CLAUDE.md's "Automation ladder — start low" principle. Mitigations: two-class hard-stop contract (ambiguity / destructive), Confirmation Contract on fallback inference, recording every workflow step through the Agent's native task tracker (so the user can interrupt at that surface), ship's hook-enforced budget, `mode=step` escape hatch.
