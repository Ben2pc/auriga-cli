...
auriga-cli 的技能和插件要服务使用**不同编码 Agent** 的同事。当前会安装这些资产的宿主是 Claude Code、Codex 和 Cursor；散文里仍不要枚举宿主。`auriga-workflow` 与 `quality-gate-scaffolder` 会进入上述三个市场；`auriga-notify` 只给 Claude Code，`session-instructions-loader` 只给 Codex。任何"默认 Agent 就是 Claude"的散文或工具假设,都会让其他运行时的同事踩坑。
...
5. **项目指令默认只写 `AGENTS.md`,必要时再补 `CLAUDE.md` 兼容入口。** Claude Code 2.1.277 起可以原生读取 `AGENTS.md`;仅当目标会话无法获得该能力,或确有 Claude 专属指令时才增加 `CLAUDE.md`。只写 `CLAUDE.md` 会让其他宿主找不到指令。

6. **散文里不枚举 Agent。** "粘回 Claude / Codex" 会随第三个 Agent 的出现而过期 —— 写 "the Agent"。

7. **Agent Plugins 1.0.0 的可移植核心与宿主能力分层维护。** 不含 Hook 的 auriga-cli 自有插件可在插件根目录提供根 `plugin.json`，并遵守闭合顶层字段；标准组件只从固定的 `skills/` 与 `mcp.json` 发现，缺少任一位置都是合法状态。包含 Hook 的插件暂时不提供根 `plugin.json`，避免 Codex 选择标准入口后跳过 `hooks/hooks.json`；`interface`、Hook 路径等宿主专属字段继续放在适用的 `.claude-plugin/plugin.json`、`.codex-plugin/plugin.json`、`.cursor-plugin/plugin.json` 中。存在标准清单时，宿主字段不能复制到其顶层，也不能把标准包身份误写成跨宿主行为支持。

## 合理例外

下列是真正的 Agent 专属,保留它们是正确的,不算违反约定:

- **成对的分运行时实现** —— `analyzers/claude-code.mjs` 配 `analyzers/codex.mjs`、`.claude-plugin/` 配 `.codex-plugin/` 配 `.cursor-plugin/` manifest；这些宿主清单是独立运行时入口，不属于 Agent Plugins 1.0.0 可移植核心。
