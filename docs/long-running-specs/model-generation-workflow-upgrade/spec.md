...
本总规范跨多个 PR 持续维护，只保存共同目标、全局约束、切片顺序和状态矩阵。每项技能开始实现前仍需在 `docs/specs/` 建立当前 PR 独有的子规范与验收契约；子 PR 进入 Ready 前，由人工决定将这些产物归档到对应 worklog、删除，或晋升为长期资料，并据此更新本总规范的状态和引用。全部子 PR 结束后，由人工决定将本总规范归档或提炼为稳定文档。
