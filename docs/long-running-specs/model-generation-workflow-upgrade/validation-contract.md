...
### VAL-REV-001
- **Behavior (行为)**: 每项进入深入评审的技能都具有唯一的主处置结论，并附有针对目标模型的证据；用户明确按分发与触发边界保留的非强制工具型技能，可以记录范围理由、风险和重新评估条件后跳过深入评审。
- **Tool (工具)**: manual
- **Evidence (判据)**: 每份完成的深入评审记录包含结论、模型证据、风险和重新评估触发条件；每项跳过深入评审的保留决定明确区分默认安装与默认执行，并声明其不构成模型能力结论。

### VAL-REV-002
- **Behavior (行为)**: 被保留或内化的强制约束都能证明其职责不能稳定地交给目标模型原生完成；按范围决定保留的工具型技能只提供能力，不被表述为强制约束。
- **Tool (工具)**: manual
- **Evidence (判据)**: 深入评审记录展示可复现失效模式，或证明该约束属于确定性机制、持久契约、独立评估或跨运行时适配；跳过深入评审的记录证明该技能不会成为每个任务的强制步骤。
