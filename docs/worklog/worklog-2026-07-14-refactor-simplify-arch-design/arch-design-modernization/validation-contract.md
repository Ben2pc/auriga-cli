...
### VAL-ARCH-007
- **Behavior (行为)**: `arch_design.md` 将影响架构选择的技术质量目标作为人工重点评审内容，示例属性不构成必填清单；同时把关键接口映射到内部接缝，让数据流在保留必要外部上下文的同时深入代码内部。部署与运行设计保持可选，普通前端或移动端功能与模块设计默认省略，只在运行结构发生变化时记录。
- **Tool (工具)**: repo-check
- **Evidence (判据)**: 技能和模板单独呈现技术质量目标及设计响应，明确示例仅用于召回；接口表引用上游依据并说明内部技术形态，数据流提示覆盖用户或外部触发方到模块、关键对象或文件级职责的完整链路；部署与运行章节明确标为可选，并写出普通前端或移动端设计的默认省略规则与例外触发条件。
