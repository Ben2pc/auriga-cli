...
> 用一个 Auriga 自有的最简测试驱动开发技能，替代外部 Superpowers 版本与独立 `test-designer` 入口。

## Why (为什么做)

新一代编码模型已经普遍具备测试先行、红绿循环和回归验证的基础能力。继续加载一份长篇、绝对化的测试驱动开发技能，会重复模型已有知识，并诱导代理为了遵守流程而增加测试数量，而不是选择最有价值的失败证据。

当前外部技能要求所有功能、缺陷修复和重构都严格先写失败测试；若实现先出现，甚至要求删除后重来。Auriga 为补足它的测试设计能力，又增加了独立 `test-designer`，要求高强度独立代理枚举场景并产出可执行失败测试。两者叠加后，容易产生低价值用例，尤其是只断言说明文字、提示词措辞或内部实现细节的测试。

Auriga 仍需要一个很小的测试入口，用于提醒代理选择有意义的行为接缝、建立正确的失败证据并以纵向小切片推进；但它不应重新教授完整的测试驱动开发理论，也不应默认派遣独立子代理。

## Findings (调研发现)

- `.agents/skills/test-driven-development/SKILL.md` 来自 `obra/superpowers`，主文件约 279 行有效内容；它把测试驱动开发设为所有代码变更的绝对流程，并要求删除先于测试出现的实现。
- `plugins/auriga-workflow/skills/test-designer/SKILL.md` 是外部技能的补充层；它要求独立代理、最高推理强度、五类场景覆盖、可执行失败测试和十二项交付检查，再把结果交回外部测试驱动开发技能。
...
技能引导当前实现代理从验收契约、缺陷事实和项目测试规则出发，选择能验证可观察行为的最小测试接缝。在存在有价值接缝时，先建立因目标行为缺失而失败的证据，再写最小实现，并在每个纵向小切片后回归验证。
...
新技能不要求独立子代理、全新会话、指定模型档位或固定推理强度。实现代理直接负责测试设计与红绿循环；拉取请求阶段的测试质量审查和完成前验证继续作为独立兜底。
