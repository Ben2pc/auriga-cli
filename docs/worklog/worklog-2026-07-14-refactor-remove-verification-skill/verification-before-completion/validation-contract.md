...
| Range (范围) | VAL ids |
|---|---|
| 技能资产 | VAL-VAST-001 ~ 001 |
| 工作流规则 | VAL-VRUL-001 ~ 001 |
| 相邻职责 | VAL-VREF-001 ~ 001 |
| 发布同步 | VAL-VREL-001 ~ 001 |
| 清理边界 | VAL-VMIG-001 ~ 001 |
| 风险记录 | VAL-VRSK-001 ~ 001 |
| 长期状态 | VAL-VSTA-001 ~ 001 |

## Parent coverage map (父级验收覆盖映射)

| Parent VAL (父级验收项) | Child VAL (子验收项) | Status (状态) |
|---|---|---|
| VAL-REV-003 | VAL-VRSK-001 | 已记录删除后的风险、观察信号与恢复条件 |
| VAL-MIG-001 | VAL-VAST-001、VAL-VREF-001 | 独立技能职责由常驻规则和现有流程承担，不形成重复入口 |
| VAL-MIG-002 | VAL-VRUL-001、VAL-VMIG-001 | 双运行时共享最小工作流规则，既有用户副本保持人工清理 |
| VAL-MIG-003 | VAL-VREL-001 | 技能集合、安装说明与发布版本同步 |
