...
- **Tool (工具)**: e2e-cli
- **Evidence (判据)**: 真实安装包在两个运行时完成预设安装，插件技能可发现；项目级与用户级历史真实布局中的受管旧副本和对应锁记录均按迁移边界处理。Codex 项目范围安装会清理用户级 Codex 副本，但项目范围的 Claude 插件不能替代用户级 Claude 副本，因此后者必须先物化并保留对应锁记录。
...
- 2026-07-14，提交 `7ad7e34`：`npm test` 通过 517/517。新增回归覆盖无法证明失效的软链接保留、校验后并发改写恢复，以及删除中断后的恢复。
- 2026-07-14，提交 `7ad7e34`：`npm run test:e2e` 通过 8/8；双运行时预设安装场景实际执行且未跳过，确认 Claude Code 与 Codex 都能发现插件技能。项目级共享副本全部退出；用户级 Codex 副本退出；未被项目范围 Claude 插件替代的用户级 Claude 副本已物化保留，对应用户锁继续存在。
