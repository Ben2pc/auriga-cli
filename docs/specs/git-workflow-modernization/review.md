...
- `pr-ready-guard` 改为递归检查 `.planning/` 下的隔离计划、活动指针和证明文件，不再检查仓库根目录的旧版三个规划文件。
...
- 全量仓库测试：522/522 通过。
- Git 门禁测试：四组共 108/108 通过，其中 Ready 门禁 37/37 通过。
...
- **风险：并行计划残留。** `.planning/` 中任意计划状态都会阻止 Ready；同一工作树存在多个任务时，必须逐个归档或删除，不能只清理当前活动计划。
