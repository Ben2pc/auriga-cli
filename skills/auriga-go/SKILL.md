...
| `auto` | Loop steps until a hard stop | **yes** | Default. Drives forward across multiple steps until ambiguity or destructive op. `max-iter` ~10. |
| `ship` | Loop until Draft → Ready candidate | no, **Experimental** | High-autonomy. Drives spec → PR Ready autonomously with strictest defaults. **Read [`references/ship.md`](references/ship.md) before invoking.** `max-iter` ~30. |

Default to `auto` unless the user specifies otherwise or the work involves a destructive operation in scope.

## Arguments

Invocation: `/auriga-go [mode] [max-iter]`, or natural-language trigger like "按照工作流继续".

Parse `$ARGUMENTS` (the full argument string) to decide mode and budget:

- **Empty / no args** → mode = `auto`, max-iter = 10
- **First token is `step` / `auto` / `ship`** → use as mode
- **Subsequent integer** → override `max-iter`
...
|---|---|---|
| `/auriga-go` | (empty) | auto, 10 |
| `/auriga-go step` | `step` | step |
| `/auriga-go auto 20` | `auto 20` | auto, 20 |
| `/auriga-go ship` | `ship` | ship, 30 |
| `/auriga-go ship 50` | `ship 50` | ship, 50 |
| `按照工作流继续` | `按照工作流继续` | auto, 10 |
...
```
[auriga-go iter N/M] 现状：<current state> → 下一步：<next action>
```

Examples:
- `[auriga-go iter 1/10] 现状：feat/foo 分支 + Draft PR 已建 + 0 commits → 下一步：step 7 TDD 红灯 (test-designer)`
- `[auriga-go iter 3/10] 现状：测试通过 + 未推送 commits → 下一步：step 10 PR readiness (push + update body)`
...
- `auto`: `max-iter` ~10 (override by passing `auto N` as args, e.g. `/auriga-go auto 20`)
- `ship`: `max-iter` ~30 (see references/ship.md)

On budget exhaustion: terminate, summarize the loop's progress, surface the decision to the user. Don't extend the budget silently.
...
- ❌ Continuing past `max-iter` — terminate, summarize, surface to user
...
auriga-go (auto, iter 2/10):
  [auriga-go iter 2/10] 现状：spec 完整 + 待 TDD → 下一步：step 7 invoke test-designer
...
auriga-go (step, iter 1/1):
  [auriga-go iter 1/1] 现状：测试全绿 + 未推送 commits → 下一步：step 10 push + update PR body
...
```
auriga-go (auto, iter 4/10):
  [auriga-go iter 4/10] 现状：deep-review 返回 punch list（3 blocking）→ 下一步：决定处理方式
