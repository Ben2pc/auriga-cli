...
7. 增量实现：绿灯阶段对任何非平凡的实现工作调用 `incremental-impl`——多文件改动、跨文件重构、落地一个已规划的 task（来源不限：内置 Plan、`planning-with-files`、`brainstorming` spec、或用户直接给的任务）、跨切面修改、或预计要写超过 ~100 行。size 判定（XS–XL）、切片策略、按需并行派遣、片间执行纪律都由 skill 自身负责——具体规则看 skill 本身。仅当 skill 的 size gate 判定为 XS、或改动是纯文档 / 纯配置时跳过。
