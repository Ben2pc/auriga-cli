...
2. 方案计划：完成需求澄清后，先做一次**规模判定**再决定 plan 方式。**满足以下三条全部成立**才走快速开发流程（详见下文「快速开发流程」段；跳过 planning，直接进入 pre-coding / 建分支阶段）：(a) 工作落在单一模块或单一概念内；(b) 验收标准 ≤5 条 bullet；(c) 不涉及跨边界接口改动（公共 API、schema、共享模块）。把判定结果记进任务追踪器（例：「规模判定 → QDF：单模块，3 条验收，无接口改动」）。任一不成立或拿不准，就走完整路径。先显式判断——无论结论如何都要表态——工作是否架构吃重：跨多个模块、要新建或重划模块边界、或"怎么做"并不显然。若是，先跑 `arch-design` skill 做架构设计（模块拆分、依赖方向、候选权衡、迁移路径）；`arch-design` 是执行跟踪方式的前置步骤，而非其替代，同时也是"跳过 spec 的架构级重构"的入口。然后用 `AskUserQuestion` 选执行跟踪方式，要把完整菜单都摆出来，不要默认只给两项：中等复杂度、临时跟踪即可的任务用内置 Plan；长程任务用 `planning-with-files` 做本地持久的文件化跟踪；`goalify` 把工作规划成一个自驱 goal，交给 Claude Code 内置的 `/goal` 命令自主执行。计划、设计决策、技术债务应作为仓库内的版本化产物，方便后续 Agent 推理上下文。
