...
| `docs/spec/` | **`brainstorming` 输出的默认归宿。** 活跃中的功能 spec 与需求澄清——设计与实现期间持续演化的活文档。spec 沉淀为长期参考时迁到 `docs/architecture/`；否则功能交付后归档到 `docs/worklog/` | 开发期活跃，之后晋升或归档 |
| `docs/architecture/` | 稳定、长期的设计文档（模块布局、数据流、组件职责）。新条目通常由 `docs/spec/` 晋升而来 | 长期 |
