...
2. Planning: After clarification, run a **scope triage** before choosing a planning method. Apply the Quick Development Flow (see "Quick Development Flow" section below; skip planning and continue at the pre-coding / branch-creation phase) **only when all three predicates hold**: (a) the work fits within a single module or one cohesive concept; (b) acceptance criteria fit in ≤5 bullets; (c) no cross-boundary interface changes (public APIs, schemas, shared modules). Record the triage verdict in the task tracker (e.g., "scope triage → QDF: single module, 3 acceptance bullets, no interface change"). If any predicate fails or you're unsure, take the full path. First decide explicitly — and state the verdict either way — whether the work is architecture-heavy: it spans modules, creates or restructures module boundaries, or the *how* is non-obvious. If so, run the `arch-design` skill first (module decomposition, dependency direction, candidate tradeoffs, migration path); `arch-design` is a precursor to execution tracking, not a replacement for it, and is also the entry point for standalone architecture-level refactors that skip spec. Then use `AskUserQuestion` to choose an execution-tracking method, and present the full menu rather than narrowing to a default pair: built-in Plan for medium-complexity work with ephemeral tracking; `planning-with-files` for long-running tasks needing local persistent file-based tracking; `goalify` to plan the work as an autonomous goal and hand it to Claude Code's built-in `/goal` command for autonomous execution. Plans, design decisions, and tech debt should be versioned artifacts in the repo for subsequent Agent context.
