...
| `docs/spec/` | **Default destination for `brainstorming` outputs.** Active feature specs and requirement clarifications — living docs that evolve through design and implementation. Promote to `docs/architecture/` once the spec stabilizes into long-lived reference material; otherwise archive to `docs/worklog/` after the feature ships. | Active during dev, then promoted or archived |
| `docs/architecture/` | Stable, long-lived design docs (module layouts, data flows, component responsibilities). New entries usually arrive by promotion from `docs/spec/`. | Long-lived |
