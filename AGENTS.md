...
6. 验证后再说完成：完成、修复、通过或可评审均须有最后一次相关修改后的对应证据。按 `spec-design` 将交付验证记入 `validation-results.md`，不记开发试跑；收尾汇报验证结果与缺口。无文件化规格时仅在对话中汇报。
...
| `docs/long-running-specs/` | 跨多个 PR 的总规范、共同约束、切片顺序和状态矩阵；跨 PR 常驻的切片子规范及其设计输入（如 spec、arch_design、validation-contract 等，非穷举——判据是“跨多个 PR 仍被引用的常驻材料”）可放本目录子文件夹，随本目录一起归档 | 跨 PR；全部子 PR 结束后人工归档 |
...
- **持续对抗熵增**：处理评审发现时，可以完成当前修改直接必要、行为不变且影响确定的低风险局部整理；可独立交付的重构属于范围扩张，未经授权只提出建议。
...
- 默认使用当前运行时的内部 Agent 工具；只有用户明确授权调用其他命令行 Agent 时，才使用外部 Agent。内部模型覆盖不同、需要跨模型或全新上下文本身不构成外部调用授权；内部工具无法满足时说明能力缺口，不自行切换到其他命令行 Agent。
- 派遣时明确结果目标、范围、验证方式和输出要求；模型与推理强度按任务风险选择，并在运行时支持时覆盖。

<!-- AURIGA:WORKFLOW:v1 END sha256=2b04bad0c8cd3514 -->
...
- `tests/agent-plugins-standard.test.ts` 检查无 Hook 插件的标准根清单、Hook 插件的原生入口、版本同步、固定 Skills 发现位置和宿主范围。
