...
6. 验证后再说完成：任何“已完成、已修复、通过或可评审”的判断，都必须基于最后一次相关修改之后、与该判断匹配的验证结果；证据不足时如实说明缺口。文件化规格使用配套的 `validation-results.md`，每完成一个交付验证活动就记录结果，也可记录契约外的额外验证，不记录开发试跑流水账；具体格式与生命周期见 `spec-design`。任务收尾直接向用户汇报已完成的验证、实际结果与未完成或受限项目，并链接可用记录；无文件化规格时仍须给出同等摘要，不强制补建文档。
...
<!-- AURIGA:WORKFLOW:v1 END sha256=7dccbd038330842e -->
